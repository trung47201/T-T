...
<body style="color: black;">
	...
	<!--######################################################################################################### -->
	<form method="post" action="/ShopTandT/" id="myform">
		<div class="text">
			<h3>Shopping</h3>
			<p class="cool">
							...
							<td>
								<div class="checkbox-cart">
									<c:if test="${ item.key.prod.discount <= 0 }">
										<input type="checkbox" class="foo" name="${ item.key.id }"
											id="${ item.value }" value="${ item.key.prod.price }">
									</c:if>
									<c:if test="${ item.key.prod.discount > 0 }">
										<input type="checkbox" class="foo" name="${ item.key.id }"
											id="${ item.value }"
								...
								<div class="price-cart">
									<c:if test="${ item.key.prod.discount > 0 }">
										<div class="flex">
											$
											<div class="price${ item.key.id }"
												id="${ item.key.prod.price - item.key.prod.price * item.key.prod.discount/100 }">
									...
									<c:if test="${ item.key.prod.discount <= 0 }">
										<div class="flex">
											$
											<div class="price${ item.key.id }"
												id="${ item.key.prod.price }">${ item.key.prod.price }</div>
										</div>
									</c:if>
								</div>
								<div class="amount-cart">
									<input class="minus-plus" type="button" name="${ item.key.id }"
										id="minus" onclick="amount(this)" value="-"> <input
										class="input_Id" type="text" id="input_Id${ item.key.id }"
										name="${ item.key.id }" value="${ item.value }" readonly>
									<input class="minus-plus" type="button" name="${ item.key.id }"
										id="plus" onclick="amount(this)" value="+">
								</div>
	...
	$(".a-login").click(function () {
		...
		xhr.open("GET", "/ShopTandT/cart?login");
		xhr.onload = function() {
		};
		xhr.send();
	});
	...
	<script type="text/javascript">
		$("#continue-shopping").click(function () {
			...
			$("#myform").attr("action", url + "/ShopTandT/products/new-arrivals");
